...
def o2m_balance_weight(indices, num_queries, K=6):
    batch_weights = []
    # indices is a list of length B
    for pred_inds, gt_inds in indices:
        # Initialize weights for all predictions in this batch element as 1.
        weights = torch.ones(num_queries, device=pred_inds.device, dtype=torch.float)
        # Compute unique GT indices and count how many predictions are matched to each GT.
        unique_gt, counts = torch.unique(gt_inds, return_counts=True)
        
        for gt_val, count in zip(unique_gt, counts):
            count_val = count.item()
            # If count > K, then set weight = K / count, else weight remains 1.
            if count_val > K:
                weight_val = K / count_val
            else:
                weight_val = 1.0
            # Find indices where this GT is matched.
            mask = (gt_inds == gt_val)
            selected_pred = pred_inds[mask]
            # Assign computed weight_val to the corresponding predictions.
            weights[selected_pred] = weight_val
        
        batch_weights.append(weights)
    
    # Stack all batch weights into a tensor of shape [B, num_queries]
    return torch.stack(batch_weights, dim=0)
...
def subsample_labels(
    ...
    """
    positive = nonzero_tuple((labels != -1) & (labels != bg_label))[0] # 100
    negative = nonzero_tuple(labels == bg_label)[0] # 200

    num_pos = int(num_samples * positive_fraction) #75
    # protect against not enough positive examples
    num_pos = min(positive.numel(), num_pos) # 75
    num_neg = num_samples - num_pos # 225
    # protect against not enough negative examples
    num_neg = min(negative.numel(), num_neg) # 200
...
class Stage2Assigner(nn.Module):
    def __init__(self, threshold=0.4, k=6, coef_box=0.7, coef_cls=0.3, positive_fraction=1.0, use_varif=False, allow_low_quality_matches=True):
        super().__init__()
        self.positive_fraction = positive_fraction  # 25% subsampling sometimes led to unstable training.
    ...
    @torch.no_grad()
    def get_cost_matrix(self, pred_logits, pred_boxes, gt_classes, gt_boxes, return_iou=False):
        ...
        if return_iou:
            C_iou = cost_box.view(num_queries, -1)
            return C.T, C_iou.T
        return C.T

    @torch.no_grad()
    def get_iou_matrix_only(self, outputs, targets):
        ...
        with torch.no_grad():
            for b in range(bs):
                ...
                cost_matrix, iou_matrix = self.get_cost_matrix(pred_logits, pred_boxes, gt_classes, gt_boxes, return_iou=True)
